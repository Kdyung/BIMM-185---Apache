...
Setting up DAVID Client:
    The DAVID Web service requires suds. If suds is not installed, install suds by downloading version 0.4 or later and following the documentation, or go to the suds folder and install using setup.py with the following command.
        >
    If suds is installed, run DAVIDclient.py with the following command
        >
    Evecuting the DAVID Web Service requires registration to the DAVID web site. Currently registered with email 'kyung.ucsd.edu'. The python code is modified for this fact.
    After running python script, the program will be able to access the Web service programmatically.
